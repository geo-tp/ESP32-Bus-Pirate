(() => {
  ...
  style.textContent = `
    ...
    @media (max-width:760px) {
      :is(.page-hero-top,.article-hero-top,.protocol-header,.site-header) { position:relative; top:auto; left:auto; right:auto; z-index:1000; overflow:visible; border-bottom:0; background:transparent; box-shadow:none; }
      .mobile-topbar-clone { position:fixed !important; top:0; left:var(--mobile-topbar-left, 6px); right:var(--mobile-topbar-right, 6px); z-index:1000; box-sizing:border-box; width:auto !important; min-width:0; max-width:none; margin-inline:0; padding:8px 0; overflow:visible; border-bottom:1px solid rgba(47,58,47,.78); background:rgba(18,18,18,.96); box-shadow:0 10px 24px rgba(0,0,0,.24); opacity:0; pointer-events:none; transform:translate3d(0,-110%,0); transition:transform 140ms cubic-bezier(.2,0,0,1), opacity 70ms linear; will-change:transform; }
      .mobile-topbar-clone.site-header { padding:8px 84px 8px 0; }
      .mobile-topbar-clone::after { display:none !important; }
      body.mobile-topbar-stuck .mobile-topbar-clone { opacity:1; pointer-events:auto; transform:translate3d(0,0,0); }
      html[data-theme="light"] .mobile-topbar-clone { border-bottom-color:rgba(202,218,209,.9); background:rgba(247,251,248,.96); box-shadow:0 10px 24px rgba(30,70,50,.12); }
      ...
      .mobile-topbar-clone .theme-nav-cluster.is-nav-open > :is(.page-top-nav,.site-nav) { position:fixed; top:calc(var(--mobile-topbar-height, 50px) + 8px); right:max(var(--mobile-topbar-right, 6px), 6px); }
      .theme-nav-cluster.is-nav-open > :is(.page-top-nav,.site-nav) a { justify-content:flex-start; width:100%; white-space:nowrap; }
      .site-header { padding-right:84px; }
      .site-header > .theme-nav-cluster { position:absolute; top:0; right:0; z-index:20; gap:6px; margin-left:0; }
      .mobile-topbar-clone:is(.article-hero-top,.site-header) > .theme-nav-cluster { top:8px; }
    ...
    @media (min-width:761px) {
      .desktop-topbar-clone {
        position:fixed !important;
        top:0;
        left:var(--desktop-topbar-left, 24px);
        right:var(--desktop-topbar-right, 24px);
        z-index:1000;
        isolation:isolate;
        display:flex;
        align-items:center;
        ...
        transition:transform 140ms cubic-bezier(.2,0,0,1), opacity 70ms linear;
        will-change:transform;
        contain:layout style;
      }
      .desktop-topbar-clone::before {
        content:"";
        position:absolute;
        top:0;
        bottom:0;
        left:calc(0px - var(--desktop-topbar-left, 24px));
        right:calc(0px - var(--desktop-topbar-right, 24px));
        z-index:0;
        pointer-events:none;
        border-bottom:1px solid rgba(255,255,255,.065);
        background:
          linear-gradient(90deg, transparent 0%, rgba(255,255,255,.012) 18%, rgba(255,255,255,.022) 50%, rgba(255,255,255,.012) 82%, transparent 100%),
          rgba(18,18,18,.985);
        box-shadow:0 8px 18px rgba(0,0,0,.18), 0 1px 0 rgba(255,255,255,.018);
      }
      .desktop-topbar-clone::after { display:none !important; }
      .desktop-topbar-clone > * { position:relative; z-index:1; }
      body.desktop-topbar-reveal .desktop-topbar-clone {
        opacity:1;
        pointer-events:auto;
        transform:translate3d(0,0,0);
      }
      html[data-theme="light"] .desktop-topbar-clone {
        border-bottom-color:transparent;
        background:transparent;
        box-shadow:none;
      }
      html[data-theme="light"] .desktop-topbar-clone::before {
        border-bottom-color:rgba(31,43,36,.085);
        background:
          linear-gradient(90deg, transparent 0%, rgba(31,43,36,.014) 18%, rgba(31,43,36,.024) 50%, rgba(31,43,36,.014) 82%, transparent 100%),
          rgba(247,251,248,.985);
        box-shadow:0 8px 18px rgba(30,42,36,.07), 0 1px 0 rgba(31,43,36,.016);
      }
      .desktop-topbar-clone .theme-nav-cluster { margin-left:auto; }
      .desktop-topbar-clone .nav-menu-toggle { display:none; }
    }
    @media (max-width:760px) {
      .desktop-topbar-clone { display:none; }
    }
    @media (min-width:761px) {
      .mobile-topbar-clone { display:none; }
    }
    ...
    html[data-theme="light"] :is(.protocol-page,.protocol-header,.protocol-hero,.protocol-band) { color:#183128 !important; }
    html[data-theme="light"] .protocol-hero { background:transparent !important; box-shadow:none !important; }
    html[data-theme="light"] .hero-visual { background:linear-gradient(145deg,#ffffff,#edf4ef) !important; border-color:#b9c9c0 !important; box-shadow:0 12px 28px rgba(31,72,53,.10), inset 0 0 5px rgba(28,57,43,.05) !important; }
    html[data-theme="light"] .hero-visual img { opacity:.82; filter:saturate(.9) brightness(1.08); }
    html[data-theme="light"] :is(.intent-card,.signal-card,.destination-card,.protocol-hub-card,.recipe-index-grid a,.note-card,.problem-card,.faq-list details,.step-list li) { background:linear-gradient(145deg,#ffffff,#f0f6f2) !important; border-color:#b9c9c0 !important; color:#183128 !important; box-shadow:0 8px 22px rgba(31,72,53,.08), inset 0 0 4px rgba(28,57,43,.05) !important; }
    html[data-theme="light"] :is(.intent-card,.signal-card,.note-card,.problem-card,.step-list li) { background:radial-gradient(circle at 0 0, rgba(0,127,122,.055), transparent 15rem), linear-gradient(145deg,#ffffff,#eef6f1) !important; border-color:#a8c5b7 !important; box-shadow:0 11px 24px rgba(31,72,53,.10), inset 0 0 4px rgba(28,57,43,.04) !important; }
    html[data-theme="light"] .intent-card::before,
    html[data-theme="light"] .signal-card::before,
    html[data-theme="light"] .note-card::before,
    html[data-theme="light"] .problem-card::before,
    html[data-theme="light"] .step-list li::before { background:linear-gradient(90deg, rgba(0,127,122,.42), rgba(8,115,69,.14), transparent) !important; }
    html[data-theme="light"] .destination-card { background:radial-gradient(circle at 100% 0, rgba(0,127,122,.055), transparent 14rem), linear-gradient(145deg,#ffffff,#eef6f1) !important; border-color:#a8c5b7 !important; box-shadow:0 12px 26px rgba(31,72,53,.10), inset 0 0 4px rgba(28,57,43,.04) !important; }
    html[data-theme="light"] .destination-card::before { background:linear-gradient(90deg, rgba(0,127,122,.40), rgba(8,115,69,.14), transparent) !important; }
    html[data-theme="light"] .faq-list details { background:linear-gradient(90deg, rgba(0,127,122,.05), transparent 42%), linear-gradient(145deg,#ffffff,#f0f6f2) !important; box-shadow:0 10px 22px rgba(31,72,53,.09), inset 0 0 4px rgba(28,57,43,.05) !important; }
    html[data-theme="light"] .faq-list details::before { background:linear-gradient(180deg, rgba(0,127,122,.42), transparent) !important; }
    html[data-theme="light"] .protocol-hub-card { background:linear-gradient(145deg,#f2fffb,#ffffff) !important; border-color:#7dbbad !important; border-left-color:#007f7a !important; box-shadow:0 12px 26px rgba(0,127,122,.12), inset 0 0 4px rgba(28,57,43,.04) !important; }
    html[data-theme="light"] .protocol-hub-card em { background:#e6f5ef !important; border-color:#7dbbad !important; color:#007f7a !important; }
    html[data-theme="light"] .protocol-hub-card::after { color:#087345 !important; }
    html[data-theme="light"] .protocol-hub-card:hover,
    html[data-theme="light"] .protocol-hub-card:focus-visible { border-color:#007f7a !important; box-shadow:0 16px 32px rgba(0,127,122,.16), inset 0 0 4px rgba(255,255,255,.7) !important; }
    html[data-theme="light"] .recipe-index-grid a { background:linear-gradient(145deg,#f2fffb,#ffffff) !important; border-color:#7dbbad !important; border-left-color:#007f7a !important; box-shadow:0 12px 26px rgba(0,127,122,.12), inset 0 0 4px rgba(28,57,43,.04) !important; }
    html[data-theme="light"] .recipe-index-grid a::after { background:#e6f5ef !important; border-color:#7dbbad !important; color:#007f7a !important; }
    html[data-theme="light"] .recipe-index-grid a:hover,
    html[data-theme="light"] .recipe-index-grid a:focus-visible { border-color:#007f7a !important; box-shadow:0 16px 32px rgba(0,127,122,.16), inset 0 0 4px rgba(255,255,255,.7) !important; }
    html[data-theme="light"] :is(.intent-card span,.signal-card span,.protocol-hub-card span,.recipe-index-grid span,.note-card span,.problem-card span,.step-list span) { color:#007f7a !important; }
    html[data-theme="light"] :is(.intent-card span,.signal-card span,.note-card span,.problem-card span) { background:#eef8f4 !important; border-color:#a8c5b7 !important; }
    html[data-theme="light"] .step-list span { background:#e6f1ea !important; border-color:#a8c5b7 !important; box-shadow:none !important; }
    html[data-theme="light"] :is(.intent-card h3,.signal-card h3,.destination-card h3,.protocol-hub-card strong,.recipe-index-grid strong,.faq-list summary) { color:#087345 !important; }
    html[data-theme="light"] :is(.lead,.section-intro,.step-list p,.intent-card p,.signal-card p,.destination-card p,.note-card p,.problem-card p,.protocol-hub-card small,.recipe-index-grid small,.faq-list p,.command-note) { color:#355448 !important; }
    html[data-theme="light"] .command-block { background:#edf5f0 !important; border-color:#9db9aa !important; color:#123e2e !important; box-shadow:inset 0 0 8px rgba(28,57,43,.08) !important; }
    html[data-theme="light"] :is(.action-link,.destination-card .action-link,.site-footer a,.top-nav a,.page-top-nav a) { background:#f8fbf9 !important; border-color:#9db9aa !important; color:#17463a !important; box-shadow:inset 0 0 4px rgba(28,57,43,.05) !important; }
    html[data-theme="light"] :is(.action-link,.destination-card .action-link,.site-footer a,.top-nav a,.page-top-nav a):hover,
    html[data-theme="light"] :is(.action-link,.destination-card .action-link,.site-footer a,.top-nav a,.page-top-nav a):focus-visible { background:#e6f1ea !important; border-color:#007f7a !important; color:#075f3b !important; box-shadow:0 8px 18px rgba(7,115,69,.10), inset 0 0 4px rgba(255,255,255,.55) !important; }
    html[data-theme="light"] .destination-card.destination-card .action-link { background:#f3faf6 !important; border-color:#9db9aa !important; box-shadow:0 7px 16px rgba(31,72,53,.08), inset 0 0 4px rgba(28,57,43,.04) !important; }
    html[data-theme="light"] .destination-card.destination-card .action-link:hover,
    html[data-theme="light"] .destination-card.destination-card .action-link:focus-visible { background:#e6f5ef !important; border-color:#007f7a !important; box-shadow:0 9px 20px rgba(0,127,122,.12), inset 0 0 4px rgba(255,255,255,.65) !important; }
    html[data-theme="light"] .site-footer { border-color:#b9c9c0 !important; color:#52665c !important; }
  ...
  const topbarSelector = ".page-hero-top:not(.mobile-topbar-clone):not(.desktop-topbar-clone), .article-hero-top:not(.mobile-topbar-clone):not(.desktop-topbar-clone), .protocol-header:not(.mobile-topbar-clone):not(.desktop-topbar-clone), .site-header:not(.mobile-topbar-clone):not(.desktop-topbar-clone)";
  let mobileTopbarClone = null;
  let desktopTopbarClone = null;

  const syncCloneTheme = (clone) => {
    if (!clone) {
      return;
    }

    const current = root.dataset.theme || "dark";
    clone.querySelectorAll(".theme-toggle").forEach((cloneButton) => {
      cloneButton.dataset.theme = current;
      cloneButton.setAttribute("aria-pressed", String(current === "light"));
      cloneButton.setAttribute("aria-label", current === "light" ? "Switch to dark theme" : "Switch to light theme");
    });
  };

  const syncAllCloneThemes = () => {
    syncCloneTheme(mobileTopbarClone);
    syncCloneTheme(desktopTopbarClone);
  };

  const setupMobileTopbar = () => {
    const media = window.matchMedia("(max-width: 760px)");
    let lastScrollY = window.scrollY;
    let ticking = false;

    const getTopbar = () => document.querySelector(topbarSelector);

    const setCloneMenuOpen = (isOpen) => {
      if (!mobileTopbarClone) {
        return;
      }

      const cluster = mobileTopbarClone.querySelector(".theme-nav-cluster");
      const menuButton = mobileTopbarClone.querySelector(".nav-menu-toggle");
    ...
    const ensureClone = () => {
      ...
      if (!topbar || mobileTopbarClone) {
        return mobileTopbarClone;
      }

      mobileTopbarClone = topbar.cloneNode(true);
      mobileTopbarClone.classList.add("mobile-topbar-clone");
      mobileTopbarClone.removeAttribute("id");
      mobileTopbarClone.querySelectorAll("[id]").forEach((element) => element.removeAttribute("id"));
      mobileTopbarClone.inert = true;
      mobileTopbarClone.addEventListener("click", (event) => {
        ...
        if (event.target.closest(".nav-menu-toggle")) {
          const cluster = mobileTopbarClone.querySelector(".theme-nav-cluster");
          setCloneMenuOpen(!cluster?.classList.contains("is-nav-open"));
          return;
        }
      ...
      document.body.appendChild(mobileTopbarClone);
      syncCloneTheme(mobileTopbarClone);
      return mobileTopbarClone;
    };

    const setReveal = (isVisible) => {
      document.body.classList.toggle("mobile-topbar-stuck", isVisible);
      if (mobileTopbarClone) {
        mobileTopbarClone.inert = !isVisible;
      }
      if (!isVisible) {
        setCloneMenuOpen(false);
      }
    ...
    const syncOffset = () => {
      ...
      if (!topbar || !clone || !media.matches) {
        return;
      }

      const rect = topbar.getBoundingClientRect();
      const brandMark = topbar.querySelector(".brand-mark");
      const anchorLeft = brandMark ? brandMark.getBoundingClientRect().left : rect.left;
      clone.style.setProperty("--mobile-topbar-left", `${Math.round(anchorLeft)}px`);
      clone.style.setProperty("--mobile-topbar-right", `${Math.round(window.innerWidth - rect.right)}px`);
      clone.style.setProperty("--mobile-topbar-height", `${Math.round(clone.getBoundingClientRect().height)}px`);
    };

    const revealClone = () => {
      syncOffset();
      mobileTopbarClone?.getBoundingClientRect();
      setReveal(true);
    };
    ...
    document.addEventListener("pointerdown", (event) => {
      if (mobileTopbarClone?.querySelector(".theme-nav-cluster")?.classList.contains("is-nav-open") && !mobileTopbarClone.contains(event.target)) {
        setCloneMenuOpen(false);
      }
    });
  };
  ...
  const setupDesktopTopbar = () => {
    ...
    const ensureClone = () => {
      const topbar = getTopbar();
      if (!topbar || desktopTopbarClone) {
        return desktopTopbarClone;
      }

      desktopTopbarClone = topbar.cloneNode(true);
      desktopTopbarClone.classList.add("desktop-topbar-clone");
      desktopTopbarClone.removeAttribute("id");
      desktopTopbarClone.querySelectorAll("[id]").forEach((element) => element.removeAttribute("id"));
      desktopTopbarClone.querySelectorAll(".nav-menu-toggle").forEach((element) => element.remove());
      desktopTopbarClone.inert = true;
      desktopTopbarClone.addEventListener("click", (event) => {
        if (!event.target.closest(".theme-toggle")) {
          return;
        }
      ...
      document.body.appendChild(desktopTopbarClone);
      syncCloneTheme(desktopTopbarClone);
      return desktopTopbarClone;
    };

    const setReveal = (isVisible) => {
      document.body.classList.toggle("desktop-topbar-reveal", isVisible);
      if (desktopTopbarClone) {
        desktopTopbarClone.inert = !isVisible;
      }
    };
    ...
    const syncOffset = () => {
      ...
      const rect = topbar.getBoundingClientRect();
      clone.style.setProperty("--desktop-topbar-left", `${Math.round(rect.left)}px`);
      clone.style.setProperty("--desktop-topbar-right", `${Math.round(window.innerWidth - rect.right)}px`);
    };
